...
`init` captures durable product truth in PRODUCT.md. It does not invent a
visual world and does not write DESIGN.md. For new direction,
[new-work.md](new-work.md) delegates creative authority to `frontend-design`;
[document.md](document.md) records an incumbent system. Existing runnable web
projects may also receive `.impeccable/live/config.json`.
...
- **Redesign/rebrand request:** preserve confirmed product truth unless the user
  changes it. `frontend-design` chooses any replacement direction later;
  new-work coordinates its persistence and implementation.
...
- Empty or early project: ask naturally for the surface to be built, or use
  `/impeccable shape <surface>` when the user wants a confirmed brief without
  implementation. New-work delegates to `frontend-design` only when the
  requested work needs unresolved visual direction.
...
If init was invoked by another request, resume without rerunning context.mjs;
the native reference above is the one thing that run could not have given you.
`frontend-design` owns later creative decisions; new-work coordinates the
handoff.
