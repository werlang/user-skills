  ...
  if (!ctx.hasProduct) {
    ...
    const parts = ctx.hasVisualImplementation
      ? [
          ...
          'simulated user. Init writes product truth only; frontend-design owns unresolved creative decisions and reference/new-work.md coordinates their implementation.',
      ...
      : [
          ...
          'must finish reference/init.md for PRODUCT.md, then reference/new-work.md delegates unresolved creative direction to frontend-design. Scoped ' +
  ...
  if (!ctx.hasDesign) {
    parts.push(ctx.hasVisualImplementation
      ? 'INCUMBENT_WORLD_UNDOCUMENTED: PRODUCT.md exists and DESIGN.md is missing, but code contains incumbent visual decisions. ' +
        ...
        'a redesign delegates replacement direction to frontend-design and uses the old look only as evidence and anti-reference. Narrow refinement ' +
      ...
      : 'WORLD_DISCOVERY_REQUIRED: PRODUCT.md exists but no DESIGN.md or incumbent visual implementation was found. ' +
        'For a new build or redesign, load reference/new-work.md and delegate unresolved creative direction to frontend-design ' +
        'with the human or structured simulated user before implementation. Scoped fixes to existing code do not need this flow.');
  }
...
function appendDetectorFallback(parts, ctx) {
  ...
  parts.push([
    ...
    'Run it once, and not before the creative direction is confirmed.',
  ].join(' '));
}
