...
export class Mysql {
    ...
    static async find(table, { filter = {}, view = ['*'], opt = {} } = {}) {
        ...
        const projection = view.length === 1 && view[0] === '*' ? '*' : view.map(v => Mysql.#quoteIdentifier(v)).join(',');
        const where = Mysql.getWhereStatements(filter);
        ...
        const sql = `SELECT ${projection} FROM \`${table}\` ${where.statement} ${order} ${limit} ${offset}`;
        return Mysql.#query(sql, where.values);
    }
